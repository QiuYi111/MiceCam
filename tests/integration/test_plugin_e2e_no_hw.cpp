...
class PluginE2ENoHwTest : public ::testing::Test {
protected:
...
TEST_F(PluginE2ENoHwTest, FullLifecycle) {
    auto* stub = plugin_.stub();

    micecam::plugin::HandshakeRequest hs_req;
    hs_req.set_micecam_version("2.0.0");
    hs_req.set_plugin_api_version(2);
    micecam::plugin::HandshakeResponse hs_resp;
    grpc::ClientContext hs_ctx;
    hs_ctx.set_deadline(std::chrono::system_clock::now() + kTimeout);
    auto hs_status = stub->Handshake(&hs_ctx, hs_req, &hs_resp);
    ASSERT_TRUE(hs_status.ok()) << hs_status.error_message();
    EXPECT_TRUE(hs_resp.accepted());
    EXPECT_EQ(hs_resp.negotiated_api_version(), 2u);
    EXPECT_EQ(hs_resp.plugin_name(), "MiceCam FFmpeg Capture");

    {
        ...
        ctx.set_deadline(std::chrono::system_clock::now() + kTimeout);
        auto s = stub->GetCapabilities(&ctx, req, &resp);
...

    {
        ...
        grpc::ClientContext ctx;
        ctx.set_deadline(std::chrono::system_clock::now() + kTimeout);
        auto s = stub->EnumerateDevices(&ctx, req, &resp);
        ASSERT_TRUE(s.ok()) << s.error_message();
    }

    {
        ...
        auto s = stub->Calibrate(&ctx, req, &resp);
        ASSERT_TRUE(s.ok()) << s.error_message();
    ...
    std::string stream_id;
    {
        ...
        cfg->set_width(640);
        cfg->set_height(480);
        cfg->set_framerate(30);
        req.set_ring_slot_count(8);
        req.set_ring_slot_size(65536);
        micecam::plugin::OpenStreamResponse resp;
        grpc::ClientContext ctx;
        ctx.set_deadline(std::chrono::system_clock::now() + kTimeout);
        auto s = stub->OpenStream(&ctx, req, &resp);
        ASSERT_TRUE(s.ok()) << s.error_message();
        ASSERT_TRUE(resp.success()) << resp.error().user_message();
        ...
        stream_id = resp.ring_descriptor().stream_id();
        plugin_.track_shm(resp.ring_descriptor().ring_id());
    }
